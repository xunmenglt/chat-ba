...
def run_model_worker(model_name):
    ...
    if FSCHAT_MODEL_WORKERS[model_name].get("gpus"):
        gpus=FSCHAT_MODEL_WORKERS[model_name].get("gpus")
        args.append("--gpus")
        args.append(gpus)
        args.insert(0,f"CUDA_VISIBLE_DEVICES={gpus}")
    # 适配npu多卡部署
    if FSCHAT_MODEL_WORKERS[model_name].get("npus"):
        npus=FSCHAT_MODEL_WORKERS[model_name].get("npus")
        args.insert(0,f"ASCEND_RT_VISIBLE_DEVICES={npus}")
    ...
    if FSCHAT_MODEL_WORKERS[model_name].get("device"):
        args.append("--device")
        args.append(FSCHAT_MODEL_WORKERS[model_name].get("device"))
